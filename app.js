...
app.get("/",(req,res)=>{
  Journal.find().then((docs)=>{
    if(docs.length===0)
    {
      res.render("home",{Hcontent:homeStartingContent,posts:posts})
    }
    else{
      res.render("home",{Hcontent:homeStartingContent,posts:docs})
    }
  });
});
app.post("/compose",(req,res)=>{
  const addjournal = new Journal({
...
app.get("/posts/:newpost",(req,res)=>{
  const requested=_.lowerCase(req.params.newpost);
  posts.forEach(post => {
